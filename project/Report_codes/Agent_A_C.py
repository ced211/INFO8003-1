...
with tf.Session() as sess:
    ...
    for episode in range(num_episodes):
        ...
        history_mean.append(np.mean(episode_history))
        if counter == 150:
            counter2 = episode
            #save_path = saver.save(sess, "Models/my_test_model"+str(counter2))
            plt.plot(episode_history, 'b')
            plt.plot(history_mean, 'r')
            plt.title("Cumulative reward as a function of the number of episodes")
