...
class Continuous_Agent:
	...
	def Dlog(self, reward_sum_discount, episode_mean, episode_action, episode_state):
		"""
		...
		dif = [(x1 - x2)/(self.std**2)*reward_sum_discount/len(episode_action) for (x1, x2) in zip(episode_action, episode_mean)]
	...
	def train(self, EPISODE_NUMBER = 2000, batch_size = 20, TIME_LIMITE = 10000):
		...
		episode_number = 1
		reward_sum_discount = 0
		action = [0]
		dlog_list = []
		previous_observation = self.env.reset()
		sum_reward_discount_list = []
		mean_sum_reward_discount_discount_list = []
		# Training progress
		time = 0		
		while episode_number != EPISODE_NUMBER:
			# create dataset
			...
			# give the observation and reward given an action.
			observation, reward, done = self.env.step(action)
			reward_sum_discount += (reward * self.env.gamma() ** time)
			reward_sum += reward
			...
			previous_observation = observation
			if self.display:
				self.Display.draw( self.env.bar.center, self.env.fruit.center, reward_sum_discount, self.env.lives)
			time += 1
			# check if the episode of the game end or does not die after TIME_LIMITE steps.
			if done or time > TIME_LIMITE:
				nb_element = 0
				dlog = self.Dlog(reward_sum_discount, episode_mean, episode_action, episode_state)
				dlog_list.append(dlog)
				sum_reward_discount_list.append(reward_sum_discount)
				if episode_number % batch_size == 0:
					self.update(dlog_list, batch_size)
					sum_reward_discount_array = np.asarray(sum_reward_discount_list)
					mean_sum_reward_discount = np.mean(sum_reward_discount_array, axis=0)
					mean_sum_reward_discount_discount_list.append(mean_sum_reward_discount)
					dlog_list = []
					sum_reward_discount_list = []
					print(self.theta)
				reward_sum_discount = 0
		...
		return mean_sum_reward_discount_discount_list
...
class Discrete_Agent:
	...
	def Dlog(self, reward_sum_discount, episode_mean, episode_action, episode_state):
		...
		# compute the difference between the action and the mean of the gaussian model divide by the 
		# variance and multiply by the sum of reward
		dif = [((x1 - x2)/(self.std**2))*reward_sum_discount for (x1, x2) in zip(episode_action, episode_mean)]
		dlog = [a*b for a,b in zip(episode_state,dif)]
		# sum for all weights
		dlog_array = np.asarray(dlog)
		return np.sum(dlog_array, axis=0)
	...
	def evaluate(self, EPISODE_NUMBER = 50, TIME_LIMITE=10000):
		...
		while episode_number != EPISODE_NUMBER:
			...
			time += 1
			if done or time > TIME_LIMITE:
				sum_reward_list.append(reward_sum_discount)
				reward_sum = 0
				episode_number += 1
				previous_observation = self.env.reset()
				time = 0
...
if __name__ == "__main__":
	...
	continuous_mean_sum_reward_discount_discount_list = []
	...
	discrete_mean_sum_reward_discount_discount_list = []
	for i in range(20):
		print(i)
		continuous_mean_sum_reward_discount_discount_list.append(continuous_agent.train())
		continuous_mean,continuous_std = continuous_agent.evaluate()
		continuous_mean_list.append(continuous_mean)
		continuous_std_list.append(continuous_std)
		discrete_mean_sum_reward_discount_discount_list.append(discrete_agent.train())
		discrete_mean,discrete_std = discrete_agent.evaluate()
		discrete_mean_list.append(discrete_mean)
		discrete_std_list.append(discrete_std)
	np.save("continuous_std_list",np.asarray(continuous_std_list))
	np.save("continuous_mean_list",np.asarray(continuous_mean_list))
	np.save("continuous_mean_sum_reward_discount_discount_list",np.asarray(continuous_mean_sum_reward_discount_discount_list))
	np.save("discrete_std_list",np.asarray(continuous_std_list))
	np.save("discrete_mean_list",np.asarray(continuous_mean_list))
	np.save("discrete_mean_sum_reward_discount_discount_list",np.asarray(continuous_mean_sum_reward_discount_discount_list))
	# Plot the data
	plt.plot(continuous_mean_sum_reward_discount_discount_list, label='linear')
